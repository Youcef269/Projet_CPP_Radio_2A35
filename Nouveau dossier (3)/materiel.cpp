...
//****************************************materiel de type plus archeter

int materiel::materielachet(){
    QSqlQuery query;

    query.prepare("select max(myCount) from (select type,count(type) mycount from materiel group by type ) ");
...
QSqlQuery materiel::materielachetDetails(){
    QSqlQuery query, query2;

    query.prepare("select type from(select type, count(type) from materiel group by type having max(count(type))) ");
    query.exec();
    query.next();

    query2.prepare("select type, count(type) from evenement group by type having count (type)=(select max(mycount) from(select type, count(type) mycount from materiel group by type ) ) ");
    query2.bindValue(":type", query.value(3).toString());
